...
module.exports = {
    ...
    async execute(client, member) {
        try {
            ...
            let message = `${member} boosted the server!`;

            if (boostMessageData && boostMessageData.boostMessage) {
                const tier = { "TIER_1": "1", "TIER_2": "2", "TIER_3": "3", "NONE": "0" };
                message = boostMessageData.boostMessage
            ...
            const embed = EmbedBuilderUtil.success('New Boost!', message);
            ...
            Logger.error(`Error in guildMemberBoost event: ${error.message}`, 'guildMemberBoost');
            Logger.debug(error.stack, 'guildMemberBoost');
        }
    }
};
